...
const FormGeneratorV2 = ({
  ...
  isModal = false,
  formGroups,
  formConfig = { formId: "", fieldWidth: 3, onSubmit: (values) => {}, onCancel: () => {}, btnSubmitText: "OK", btnCancelText: "Annuler", loading: false, buttonsFloatBottom: false },
}) => {
  ...
  //     const element = formGroups[index];
  //   }
  formGroups.forEach((formGroup) => {
    formGroup.formGroupFields.forEach((field) => {
  ...
  return (
    <div>
      ...
      >
        ...
        <ModalBody>
          {formGroups.map((formGroup, formGroupKey) => (
            <FormGroup key={formGroupKey}>
              <div className="border-bottom mb-2">
                <h5> {formGroup.formGroupName} </h5>
              </div>
              <Row>
                {formGroup.formGroupFields.map((field, key) => (
                        ...
                        <>
                          <PhoneInput
                            ...
                            onChange={(val) => {
                              ...
                              formik.isValid = isValidPhoneNumber(formik.values[field.name]) ? true : false;
                            }}
                            onBlur={formik.handleBlur}
                          />
                        </>
